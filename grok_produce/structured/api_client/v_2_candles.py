...
#     """
#     Bitget returns rows like:
#       [ts, open, high, low, close, quoteVol, baseVol, ???]
#     We’ll use: ts, open, high, low, close, baseVol
#     and cast to the requested schema.
#     """
#     if not rows:
#         return pd.DataFrame(columns=['timestamp','open','high','low','close','volume'])
#
#     # rows are usually newest→oldest; we’ll handle sort next
#     out = []
#     for r in rows:
#         ts_ms = int(r[0])                         # milliseconds
#         o, h, l, c = map(float, r[1:5])
#         base_vol = float(r[6]) if len(r) > 6 else float(r[5])  # fallback if shape differs
...
#     return df
#
# def fetch_mix_candles(symbol: str,
#                       granularity_sec: int,
#                       count: int,
#                       product_type: str = "USDT-FUTURES",
#                       page_limit: int = 1000) -> pd.DataFrame:
#     """
#     Fetch up to `count` candles by paging /api/v2/mix/market/candles.
#     - symbol: e.g., 'BTCUSDT', 'ETHUSDT', 'DOGEUSDT', 'SOLUSDT', 'XRPUSDT'
#     - granularity_sec: 300 (5m), 14400 (4h), etc.
#     """
#     all_rows: List[List[str]] = []
#     end_time: Optional[int] = None  # ms; if None, server uses 'now'
#     remaining = count
#
#     while remaining > 0:
#         batch = min(page_limit, remaining)
#         params = {
#             "productType": product_type,     # V2 futures product type
#             "symbol": symbol,                # V2 symbol (no _UMCBL)
...
#
#         # data is newest→oldest; next page endTime should be strictly before the oldest we just got
#         oldest_ts = int(data[-1][0])  # ms
#         # step back one millisecond to avoid duplicate
#         end_time = oldest_ts - 1
#         remaining -= len(data)
#
#         # safety: if server returns fewer than requested and we’re not moving, stop
#         if len(data) < batch:
#             break
#
...
#         time.sleep(0.05)
#
#     # We may have collected a bit more than needed; trim to last `count`
#     df = _candles_to_df(all_rows)
#     if len(df) > count:
#         df = df.iloc[-count:].reset_index(drop=True)
#     return df
#
# # Convenience wrappers the way you asked:
# def get_last_4900_5m(symbol: str) -> pd.DataFrame:
#     """Last 4900 five‑minute candles (granularity 300s)."""
#     return fetch_mix_candles(symbol, granularity_sec=300, count=4900)
#
# def get_last_105_4h(symbol: str) -> pd.DataFrame:
#     """Last 105 four‑hour candles (granularity 14400s)."""
#     return fetch_mix_candles(symbol, granularity_sec=14400, count=105)
#
# # ---- Example usage ----
# if __name__ == "__main__":
#     # Pick any of: BTCUSDT, ETHUSDT, DOGEUSDT, SOLUSDT, XRPUSDT
#     sym = "BTCUSDT"
#
#     df_5m = get_last_4900_5m(sym)
#     print(sym, "5m candles shape:", df_5m.shape)
#     print(df_5m.head(), "\n", df_5m.tail())
#
#     df_4h = get_last_105_4h(sym)
#     print(sym, "4h candles shape:", df_4h.shape)
#     print(df_4h.head(), "\n", df_4h.tail())
#
#
import time
