...
from technical_indicators import calculate_rsi, calculate_bollinger_bands, calculate_volume_ma, \
    calculate_ema, calculate_stochastic_rsi, calculate_macd, calculate_atr
import logging
...
class CurrentStrategy(Strategy):
    ...
    def check_4h_confluence(self, df_4h, current_time):
        ...
        return abs(price - bb_lower) / price < 0.03  # Increased to 3%
...
class StochasticStrategy(Strategy):
    """Variant strategy: Stochastic RSI < 0.2, Bollinger Band breakout, volume spike, 4h BB only."""
    def apply_indicators(self, df, timeframe='5m'):
        df = df.copy()
